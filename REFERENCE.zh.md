...
| 阶段 | K | 误差 |
|------|---|------|
| SVM (ghost 修复后) | 20 | 2.31e-6 |
| + Refine 10 轮 | 20 | ~2.1e-6 |
| + TDVP | 20 | ~2.0e-6 |

**K=20 之后增加基函数无收益**——瓶颈在采样分布的多样性（B 和 R 参数浪费搜索空间），不在 K 大小。预期 B=0, R=0 后 K=10 就能到 1e-8。
...
|------|---|------|
| SVM | 13 | 2.42e-3 |
| + Refine 10 轮 | 13 | 2.29e-3 |
| TDVP | — | 发散 |

误差卡在 2.3e-3 是**代数收敛（K^{-1/2}）的预期行为**，不是 bug。TDVP 发散因为 C 矩阵条件数 3e+11（cusp 结构导致）。
